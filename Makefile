...
test_grammar: verify-$(TEST_GRAMMAR_FILE2) verify-$(TEST_GRAMMAR_FILE)
...
push_to_github:
	...
	rsync -aAHX --exclude .git --exclude snippets.py ./ $(TESTGITHUB)/pykythe/
	rsync -aAHX ../kythe $(TESTGITHUB)/
	-cd $(TESTGITHUB)/pykythe && git status
	-cd $(TESTGITHUB)/pykythe && git difftool --no-prompt --tool=tkdiff
	@echo '# cd $(TESTGITHUB)/pykythe && git commit -mCOMMIT-MSG' -a
	@echo '# cd $(TESTGITHUB)/pykythe && git push -u origin master'

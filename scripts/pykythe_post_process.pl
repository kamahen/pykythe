...
%% "kythe" accumulator gets Kythe facts, each a dict to be output in JSON.
edcg:acc_info(kythe, T, Out, In, Out=[T|In]).

%% All -->> predicates have the same hidden arguments:
edcg:pred_info(_, _, [kythe]).
...
kythe_json(Node, Result) :- kythe_json(Node, Result, []).  % phrase(kythe_json(...))
kythe_json(Node) -->>
    kythe_file,
    node_anchors(Node).
...
kythe_fact_b64(Source, FactName, FactBase64) -->>
    [ _{source: Source, fact_name: FactName, fact_value: FactBase64} ]:kythe.

kythe_edge(Source, EdgeKind, Target) -->>
    [ _{source: Source, edge_kind: EdgeKind, target: Target, fact_name: '/'} ]:kythe.

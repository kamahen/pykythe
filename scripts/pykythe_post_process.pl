...
% The following items from the simplification are all handled by higher-level
% nodes (e.g., 'Astn'{start: int(Start), end: int(End), value: str(Value)}):
% [], [_|_], bool(_), dict(_), int(_), none, str(_)
...
node_anchors2('NameNode'{binds: bool(_Binds), name: Name}, todo_expr(name___)) -->>
    node_anchors(Name, _).

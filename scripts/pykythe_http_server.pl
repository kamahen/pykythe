...
% -*- Mode: Prolog -*-
               ...
               %% This is a simple HTTP server for displaying the the
               %% results of running pykythe (JSON output).  There is
               %% one window for the program source and one window for
               %% the xref info.

               %% The "top" window sets up two iframe's:
               %%   "content": the source content (colorized)
               %%   "xref": for the xref (Kythe) information)

               %% The content has links (target="xref") that cause the
               %% xref information to be computed and displayed. The
               %% xref information has links (target="content") into
               %% the source.

               %% TODO: add a third iframe (called "toc" will be added
               %% for navigating through the source directory.

...
%% The test cases are in ../test_data/c3_a.py, which is a slightly
%% modified version of the algorithm, as published in the Python
%% documentation, together with test cases (which have been annotated
%% with Kythe verifier test goals).
...
mro(module_type(_ModuleType), Mro) => Mro = [].
